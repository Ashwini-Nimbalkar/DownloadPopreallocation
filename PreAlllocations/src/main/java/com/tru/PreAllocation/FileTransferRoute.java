...
public class FileTransferRoute extends RouteBuilder {
	...
	public void configure() throws Exception {
		...
		onException(Exception.class).continued(true)
				...
				.to("sql:insert into SOLOMONK.ITSESBHDR(FILENAME,TIMESENT,ESBSNTSTS,PROCESSTS,PROCESRMK) values(:#filename,:#reportTime,'S','S','Sent from ESB successfully')?dataSource=#mydata")
				.log("${header.RjctReason} in ${file:name} file").stop();

		from("{{hkgIncoming.path}}").routeId("ITSAllocation-Route").to("{{as400hkg.path}}", "{{hkgOutgoing.path}}")

				.log("file sending from ftp").setHeader("filename", simple("${file:name}"))
				.log("filename in ${file:name}")
				.setHeader("filename", simple("${file:name}"))
		        .setHeader("reportTime", simple("${date:now:yyyy-MM-dd HH:mm:ss}"))		  .to("sql:insert into SOLOMONK.ITSESBHDR(FILENAME,TIMESENT,ESBSNTSTS,PROCESSTS,PROCESRMK) values(:#filename,:#reportTime,'S','S','Sent from ESB successfully')?dataSource=#mydata");
